...
# +
iedDetector.plotFeatures(XTest, yTest, ['Power max-SNR', 'mean signal power'],
                         threshold)

print('# Confusion matrix')
confMat = confusion_matrix(yTest, predictions)
display(pd.DataFrame(confMat))
ied.plotROC(predictions_proba, yTest, confMat, sensitivity=0.6)

predictions_probaAll = iedDetector.predict(Xs)
predictionsAll = (predictions_probaAll >= threshold).astype(bool)

display(Markdown('### IED'))
spikeEvents = np.where(~selector*ys)[0]/params["fsClassify"]
for i in range(min(int(np.ceil(len(data.annotations)/25)), 4)):
    ied.plotEvents(selectedData.T,
       XsUp.T/norm, predictionsAll, data.info['sfreq'],
       params["fsClassify"], spikeEvents[i*25:min(len(spikeEvents), (i+1)*25)])

display(Markdown('### FP'))
interferenceEvents = np.where(np.logical_and((~selector*predictionsAll), ys==0))[0]/params["fsClassify"]
for i in range(min(int(len(interferenceEvents)/25), 4)):
    ied.plotEvents(selectedData.T,
               XsUp.T/norm, 1-predictionsAll, data.info['sfreq'],
